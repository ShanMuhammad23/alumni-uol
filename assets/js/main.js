...
(function ($) {
	...
	function appendNewslettersLink(listEl) {
		...
		link.textContent = "Newsletters";
		li.appendChild(link);

		const contactLink = listEl.querySelector('a[href="/contact/"]');
		if (contactLink && contactLink.parentElement) {
			listEl.insertBefore(li, contactLink.parentElement);
		} else {
 ...
 // Function to hide the parent section
	...
	// Distinguished Alumni Slider - Initialize only if content exists, otherwise will be initialized after content loads
	var distinguishedAlumniSlider;
	var distinguishedAlumniSliderEl = document.querySelector('.tp-distinguished-alumni-active');
	if (distinguishedAlumniSliderEl) {
		var wrapper = distinguishedAlumniSliderEl.querySelector('.swiper-wrapper');
		// Only initialize if wrapper has slides (content already loaded)
		if (wrapper && wrapper.children.length > 0) {
			distinguishedAlumniSlider = new Swiper('.tp-distinguished-alumni-active', {
				slidesPerView: 6,
				spaceBetween: 20,
				loop: true,
				speed: 800,
				...
				// Navigation arrows
				navigation: {
					nextEl: ".tp-distinguished-alumni-next",
					prevEl: ".tp-distinguished-alumni-prev",
				},
				// Pagination dots
				pagination: {
					el: ".tp-distinguished-alumni-pagination",
					clickable: true,
				},
				breakpoints: {
					'1400': {
			...
			window.distinguishedAlumniSlider = distinguishedAlumniSlider;
		}
	}
